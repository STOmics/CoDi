...
for lq_path in lq_paths:
    ...
    command = (
        command_gen.replace("hq_path", hq_path)
        .replace("lq_path", lq_path)
    ...
    if algo_suffix == "ssi":
        if "-d" in command_split:
            ...
            distance_metric = "KLD"
        lq_path_pred = os.path.basename(lq_path).replace(
    ...
    res_df = calc_metric(
        adata_sc.obs[annotation], adata_st.obs[algo_suffix], subsample_factor
    )
